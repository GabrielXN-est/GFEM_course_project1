...
int main()
{
    try {
        ...
        std::cout << "________________alpha = 0,5________________" << std::endl;
        {
            ...
            std::cout << "Convergence rate for h-FEM in terms of dofs: " << (std::log(h_FEM_error[size-1])-std::log(h_FEM_error[size-2]))/(std::log(L/h_FEM_dofs[size-1])-std::log(L/h_FEM_dofs[size-2])) << "\n";
            std::cout << "Convergence rate for h-GFEM in terms of dofs: " << (std::log(h_GFEM_error[size-1])-std::log(h_GFEM_error[size-2]))/(std::log(L/h_GFEM_dofs[size-1])-std::log(L/h_GFEM_dofs[size-2])) << "\n";
            std::cout << "Convergence rate for p-FEM in terms of dofs: " << (std::log(p_FEM_error[size-1])-std::log(p_FEM_error[size-2]))/(std::log(L/p_FEM_dofs[size-1])-std::log(L/p_FEM_dofs[size-2])) << "\n";
            std::cout << "Convergence rate for p-GFEM in terms of dofs: " << (std::log(p_GFEM_error[size-1])-std::log(p_GFEM_error[size-2]))/(std::log(L/p_GFEM_dofs[size-1])-std::log(L/p_GFEM_dofs[size-2])) << "\n";
            //plotagens

...
// function to simplify analysis definition
void simulation(std::string path,
const std::vector<int>& n_elem_L, std::vector<double>& error_reference, std::vector<double>& dofs_reference,
...
int main()
{
    try {
        ...
        std::vector<int> nelem_o_L{3, 5, 7, 9, 11, 13, 15, 17, 19};
            ...
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_FEM_lin_p_error, h_FEM_lin_p_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_FEM_pord_" + std::to_string(1), 1, "lBar");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_FEM_lin_o_error, h_FEM_lin_o_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_FEM_pord_" + std::to_string(1), 1, "lBar");
 
        std::cout << "________________h-version FEM quadratic________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_FEM_quad_p_error, h_FEM_quad_p_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_FEM_pord_" + std::to_string(2), 2, "lBar");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_FEM_quad_o_error, h_FEM_quad_o_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_FEM_pord_" + std::to_string(2), 2, "lBar");

        std::cout << "________________h-version GFEM linear________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_GFEM_lin_p_error, h_GFEM_lin_p_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_pord_" + std::to_string(1), 1, "pGFEMBar");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_GFEM_lin_o_error, h_GFEM_lin_o_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_pord_" + std::to_string(1), 1, "pGFEMBar");

        std::cout << "________________h-version GFEM quadratic________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_GFEM_quad_p_error, h_GFEM_quad_p_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_pord_" + std::to_string(2), 2, "pGFEMBar");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_GFEM_quad_o_error, h_GFEM_quad_o_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_pord_" + std::to_string(2), 2, "pGFEMBar");

        std::cout << "________________h-version GFEM linear Sukumar________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_GFEM_lin_p_error_suk, h_GFEM_lin_p_dofs_suk,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_S_pord_" + std::to_string(1), 1, "pGFEMBar_WD_S");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_GFEM_lin_o_error_suk, h_GFEM_lin_o_dofs_suk,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_S_pord_" + std::to_string(1), 1, "pGFEMBar_WD_S");

        std::cout << "________________h-version GFEM quadratic Sukumar________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_GFEM_quad_p_error_suk, h_GFEM_quad_p_dofs_suk,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_S_pord_" + std::to_string(2), 2, "pGFEMBar_WD_S");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_GFEM_quad_o_error_suk, h_GFEM_quad_o_dofs_suk,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_S_pord_" + std::to_string(2), 2, "pGFEMBar_WD_S");

        std::cout << "________________h-version GFEM linear Moes________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_GFEM_lin_p_error_moes, h_GFEM_lin_p_dofs_moes,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_M_pord_" + std::to_string(1), 1, "pGFEMBar_WD_M");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_GFEM_lin_o_error_moes, h_GFEM_lin_o_dofs_moes,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_M_pord_" + std::to_string(1), 1, "pGFEMBar_WD_M");

        std::cout << "________________h-version GFEM quadratic Moes________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(path, nelem_p_L, h_GFEM_quad_p_error_moes, h_GFEM_quad_p_dofs_moes,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_M_pord_" + std::to_string(2), 2, "pGFEMBar_WD_M");

            std::cout << " \n -> Odds" << std::endl;
            simulation(path, nelem_o_L, h_GFEM_quad_o_error_moes, h_GFEM_quad_o_dofs_moes,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_M_pord_" + std::to_string(2), 2, "pGFEMBar_WD_M");
        ...
        std::cout << "________________Taxas de Convergência (h-version)________________" << std::endl;
            ...
            //plotagens
            plot_error(h_FEM_lin_p_dofs, h_FEM_lin_o_dofs, h_FEM_lin_p_error, h_FEM_lin_o_error, "Degrees of freedom", "Ex2 - Standart linear FEM", path + "/plots/convergence/");
            plot_error(h_FEM_quad_p_dofs, h_FEM_quad_o_dofs, h_FEM_quad_p_error, h_FEM_quad_o_error, "Degrees of freedom", "Ex2 - Standart quadratic FEM", path + "/plots/convergence/");
            plot_error(h_GFEM_lin_p_dofs, h_GFEM_lin_o_dofs, h_GFEM_lin_p_error, h_GFEM_lin_o_error, "Degrees of freedom", "Ex2 - linear GFEM = FEM", path + "/plots/convergence/");
            plot_error(h_GFEM_quad_p_dofs, h_GFEM_quad_o_dofs, h_GFEM_quad_p_error, h_GFEM_quad_o_error, "Degrees of freedom", "Ex2 - quadratic GFEM", path + "/plots/convergence/");
            plot_error(h_GFEM_lin_p_dofs_suk, h_GFEM_lin_o_dofs_suk, h_GFEM_lin_p_error_suk, h_GFEM_lin_o_error_suk, "Degrees of freedom", "Ex2 - linear GFEM with Sukumar", path + "/plots/convergence/");
            plot_error(h_GFEM_quad_p_dofs_suk, h_GFEM_quad_o_dofs_suk, h_GFEM_quad_p_error_suk, h_GFEM_quad_o_error_suk, "Degrees of freedom", "Ex2 - quadratic GFEM with Sukumar", path + "/plots/convergence/");
            plot_error(h_GFEM_lin_p_dofs_moes, h_GFEM_lin_o_dofs_moes, h_GFEM_lin_p_error_moes, h_GFEM_lin_o_error_moes, "Degrees of freedom", "Ex2 - linear GFEM with Moes", path + "/plots/convergence/");
            plot_error(h_GFEM_quad_p_dofs_moes, h_GFEM_quad_o_dofs_moes, h_GFEM_quad_p_error_moes, h_GFEM_quad_o_error_moes, "Degrees of freedom", "Ex2 - quadratic GFEM with Moes", path + "/plots/convergence/");
        #endif
        #ifdef P_VERSION
        ...
        for (int pord: pord_L)
        {
            simulation(path, {4}, p_FEM_p_error, p_FEM_p_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
        ...
        for (int pord: pord_L)
        {
            simulation(path, {5}, p_FEM_o_error, p_FEM_o_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_2_FEM_pord_" + std::to_string(pord), pord, "lBar", tol);
        }
        ...
        for (int pord: pord_L)
        {
            simulation(path, {4}, p_GFEM_p_error, p_GFEM_p_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
        ...
        for (int pord: pord_L)
        {
            simulation(path, {5}, p_GFEM_o_error, p_GFEM_o_dofs,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_2_GFEM_pord_" + std::to_string(pord), pord, "pGFEMBar", tol);
        }
        ...
        for (int pord: pord_L)
        {
            simulation(path, {4}, p_GFEM_p_error_suk, p_GFEM_p_dofs_suk,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
        ...
        for (int pord: pord_L)
        {
            simulation(path, {5}, p_GFEM_o_error_suk, p_GFEM_o_dofs_suk,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_2_GFEM_S_pord_" + std::to_string(pord), pord, "pGFEMBar_WD_S", tol);
        }
        ...
        for (int pord: pord_L)
        {
            simulation(path, {4}, p_GFEM_p_error_moes, p_GFEM_p_dofs_moes,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
        ...
        for (int pord: pord_L)
        {
            simulation(path, {5}, p_GFEM_o_error_moes, p_GFEM_o_dofs_moes,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_2_GFEM_M_pord_" + std::to_string(pord), pord, "pGFEMBar_WD_M", tol);
        }
        ...
        std::cout << "________________Plotagens (p-version)________________" << std::endl;
            plot_error(p_FEM_p_dofs, p_FEM_o_dofs, p_FEM_p_error, p_FEM_o_error, "Degrees of freedom", "Ex2 - Standart FEM (p-version)", path + "/plots/convergence/");
            plot_error(p_GFEM_p_dofs, p_GFEM_o_dofs, p_GFEM_p_error, p_GFEM_o_error, "Degrees of freedom", "Ex2 - p-GFEM (p-version)", path + "/plots/convergence/");
            plot_error(p_GFEM_p_dofs_suk, p_GFEM_o_dofs_suk, p_GFEM_p_error_suk, p_GFEM_o_error_suk, "Degrees of freedom", "Ex2 - p-GFEM with Sukumar (p-version)", path + "/plots/convergence/");
            plot_error(p_GFEM_p_dofs_moes, p_GFEM_o_dofs_moes, p_GFEM_p_error_moes, p_GFEM_o_error_moes, "Degrees of freedom", "Ex2 - p-GFEM with Moes (p-version)", path + "/plots/convergence/");
        #endif
        return 0;
    }
    catch (const std::exception& e)

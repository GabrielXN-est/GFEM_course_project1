...
void simulation(const std::vector<int>& n_elem_L, // n de elementos a iterar
    ...
    std::string title, int porder, std::string eltype, std::vector<double> geom_enr = {}, double tol = std::pow(10, -30))
{
    ...
    for (int nelem: n_elem_L)
    {
        ...
        if (eltype == "lBar")
            ...
            generate_input(filename, nelem, 1, eltype, L, E, {s}, A, 0, // filename, nel, porder, eltype, L, E, Exlim, A, C,
            std::vector<double> {{0.,1.}}, std::vector<int> {0, 1}, std::vector<int> {1, 1}, // d_bcs, d_bcs_pos, d_bcs_dofs,
            std::vector<double> {}, std::vector<int> {}, std::vector<int> {},// f_bcs, f_bcs_pos, f_bcs_dofs,
        ...
        std::cout << "Conditioning number for " << title << " with " << nelem << " elements equals: " << conditioning_reference.back() << std::endl;
...
int main()
{
    try {
        ...
        std::cout << "________________h-version FEM________________" << std::endl;
            ...
            simulation(nelem_L, h_FEM_error, h_FEM_dofs, h_FEM_conditioning,
        ...
        std::cout << "________________h-version GFEM topological________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(nelem_L, h_GFEM_topo_error, h_GFEM_topo_dofs, h_GFEM_topo_conditioning,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_topo_pord_" + std::to_string(1), 1, "pGFEMBar_2Proj");

        std::cout << "________________h-version SGFEM topological________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(nelem_L, h_SGFEM_topo_error, h_SGFEM_topo_dofs, h_SGFEM_topo_conditioning,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_SGFEM_topo_pord_" + std::to_string(1), 1, "pSGFEMBar_2Proj");

        std::cout << "________________h-version GFEM geometrical________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(nelem_L, h_GFEM_geom_error, h_GFEM_geom_dofs, h_GFEM_geom_conditioning,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_GFEM_geom_pord_" + std::to_string(1), 1, "pGFEMBar_2Proj", {0,1/4});

        std::cout << "________________h-version SGFEM geometrical________________" << std::endl;
            std::cout << " \n -> Pairs" << std::endl;
            simulation(nelem_L, h_SGFEM_geom_error, h_SGFEM_geom_dofs, h_SGFEM_geom_conditioning,
            L, x_gamma, E, A, bf_func, U_exact, // parametros fixos
            "EX2_1_SGFEM_geom_pord_" + std::to_string(1), 1, "pSGFEMBar_2Proj", {0,1/4});
        ...
        std::cout << "________________Taxas de Convergência________________" << std::endl;
            ...
            //plotagens
            plot_error(h_FEM_dofs, h_GFEM_topo_dofs, h_SGFEM_topo_dofs, h_FEM_error, h_GFEM_topo_error, h_SGFEM_topo_error, "Degrees of freedom", "Topological enrichment error", "./plots/convergence/");
            plot_error(h_FEM_dofs, h_GFEM_geom_dofs, h_SGFEM_geom_dofs, h_FEM_error, h_GFEM_geom_error, h_SGFEM_geom_error, "Degrees of freedom", "Geometrical enrichment error", "./plots/convergence/");
            plot_error(h_FEM_dofs, h_GFEM_topo_dofs, h_SGFEM_topo_dofs, h_FEM_conditioning, h_GFEM_topo_conditioning, h_SGFEM_topo_conditioning, "Degrees of freedom", "Topological enrichment conditioning", "./plots/convergence/", true);
            plot_error(h_FEM_dofs, h_GFEM_geom_dofs, h_SGFEM_geom_dofs, h_FEM_conditioning, h_GFEM_geom_conditioning, h_SGFEM_geom_conditioning, "Degrees of freedom", "Geometrical enrichment conditioning", "./plots/convergence/", true);
        return 0;
    }
    catch (const std::exception& e)
    {

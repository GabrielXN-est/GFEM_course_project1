...
std::string list_numbers(int in)
{
    std::string result {};
    for (int i {1}; i <= in; i++)
    {
...
void generate_input(std::string filename, int nel, int porder, std::string eltype,
    ...
    int bf_func_id, double alpha, double xb, double xi, 
    double xgamma, int porder_Enrichment, std::vector<double> geom_enr, int localproblems)
{
    // flags
    ...
    // numero de enriquecimentos
    int nenr {1};
    if (enriched_interface && localproblems > 0)
        nenr = 1 + localproblems;
    else if (localproblems > 0)
        nenr = localproblems;

    // nodes description
    double xi_el {0}, xf_el {0};

    if ((enriched_interface || localproblems > 0) && geom_enr.size() != 0)
    {
        ...
        for (int i {0}; i < nnodes; i++)
        {
            ...
            if (xi_el >= geom_enr[0] && xi_el <= geom_enr[1])
                file << i+1 << " " << xi_el << " " << list_numbers(nenr) << "\n";
            else
                file << i+1 << " " << xi_el << "\n";
        }
    }
    ...
    if ((enriched_interface || localproblems > 0) && geom_enr.size() != 0)
    {
        ...
        if (eltype == "pGFEMBar_WD_S" || eltype == "pGFEMBar_WD_M")
            ...
            file << "nenrichments; enrID type local_mesh_id";
        else
        ...
        int i {0};
        if (localproblems > 0)
            {for (; i < localproblems; i++)
                {file << i+1 << " Global_local" << " " << i << "\n";}}


        if (eltype == "pGFEMBar_WD_S")
            file << i+1 << " ESuk " << xgamma << "\n";
        else if (eltype == "pGFEMBar_WD_M")
            file << i+1 << " EMoes " << xgamma << "\n";
        else if (eltype == "pGFEMBar_2Proj")
            file << i+1 << " EProject2\n";
        else if (eltype == "pSGFEMBar_2Proj")
            file << i+1 << " E_SGFEM_Project2\n";
    }

...
int main()
{
    try {
        ...
        // solução para comparação
        Mesh mesh_ex {path, "3P_bonus_GFEM_nelem_" + std::to_string(128) + "_nelem_GL_" + std::to_string(256), 128,
            ...
            bf_func, alpha, xb, 0.0, 0.0, 0,
            {3*L/8, 5*L/8}, 1};

        mesh_ex.first_run();
        mesh_ex.create_local_problem(L/4, L/2, 256);
        mesh_ex.run_local_problems();
        mesh_ex.run();

        double U_exact {mesh_ex.strain_energy()};
        ...
        std::cout << "________________h-version FEM________________" << std::endl;
            for (int nelem : nelem_L)
            {
                Mesh mesh {path, "3P_bonus_FEM_nelem_" + std::to_string(nelem), nelem,
                    1, "lBar", L, E, E_xlim, A, 0,
                    d_bcs,d_bcs_pos, d_bcs_dofs, // dirichilet boundary conditions
                    f_bcs, f_bcs_pos, f_bcs_dofs, // Neumann Boundary conditions
                    bf_func, alpha, xb};
        ...
        for (int i = 0; i < nelem_L_GL.size(); i++)
        {
            ...
            for (int nelem : nelem_L)
            {
                Mesh mesh {path, "3P_bonus_GFEM_nelem_" + std::to_string(nelem) + "_nelem_GL_" + std::to_string(nelem_GL), nelem,
                    1, "lBar", L, E, E_xlim, A, 0,
                    d_bcs,d_bcs_pos, d_bcs_dofs, // dirichilet boundary conditions
                    f_bcs, f_bcs_pos, f_bcs_dofs, // Neumann Boundary conditions
                    bf_func, alpha, xb, 0.0, 0.0, 0,
        ...
        std::cout << "________________Plotagem________________" << std::endl;
            ...
            for (double x {0}; x <= L; x += L/1000.)
            {
                ...
                u_ex_values.push_back(mesh_ex.interpolate_solution(x));
            }
            ...
            plot_error(h_FEM_dofs, h_GFEM_GL_h_L_64_dofs, h_GFEM_GL_h_L_128_dofs, h_GFEM_GL_h_L_256_dofs,
                       ...
                       "Degrees of freedom", "Error in respect to degrees of freedom bonus (bonus problem)", path + "/plots/convergence/");
            plot_error(nelem_L, nelem_L, nelem_L, nelem_L,
                       h_FEM_error, h_GFEM_GL_h_L_64_error, h_GFEM_GL_h_L_128_error, h_GFEM_GL_h_L_256_error,
                       "Global element size", "Error in respect to element size (bonus problem)", path + "/plots/convergence/");
            ...
            matplot::ylabel("u(x)");
            matplot::legend();
            matplot::save(path + "/plots/solutions_bonus" + ".png");
        return 0;
    }
